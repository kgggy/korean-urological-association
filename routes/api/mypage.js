    ...
    try {
        ...
        connection.query(sql, param, async (err) => {
            ...
            if (req.files != null) {
                ...
                //바뀐 이미지 있는경우
                var deleteFileRoute = req.body.deleteFileRoute; //바꾼 파일(바뀌기 전 경로)
                console.log("deleteFileRoute =============== " + deleteFileRoute);
                if (deleteFileRoute != '') {
                    var deleteFileRoute = [deleteFileRoute]
                    var aa = deleteFileRoute.split(',')
                    console.log(aa)
                    ...
                    //파일 테이블에서 기존꺼 삭제
                    for (var i = 0; i < deleteFilerouteArr.length; i++) {
                        const fileDelSql = "delete from file where fileRoute = ?;"
                        const fileDelSqlParam = deleteFilerouteArr[i];
